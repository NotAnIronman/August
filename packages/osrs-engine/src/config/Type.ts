...
export abstract class Type {
    ...
    decode(buffer: ByteBuffer): void {
        while (true) {
            // For DAT2 (per-file) config entries, reaching EOF can be treated as
            // an implicit terminator to tolerate empty or truncated files.
            if (buffer.offset >= buffer.length) {
                if (this.cacheType === "dat2") {
                    break;
                }
            ...
            try {
                ...
                // Gracefully stop on mid-opcode EOF for DAT2 entries
                if (this.cacheType === "dat2" && /Buffer overflow/.test(e?.message ?? "")) {
                    break;
                }
                throw e;
            }
