...
import { ArchiveFile } from "@august/osrs-engine/cache/ArchiveFile";
import { Container } from "@august/osrs-engine/cache/Container";
import { IndexType } from "@august/osrs-engine/cache/IndexType";
import { ArchiveReference } from "@august/osrs-engine/cache/ref/ArchiveReference";
...
export abstract class CacheIndex<A extends ApiType = ApiType.SYNC> {
    ...
    getFileSmart(id: number, key?: number[]): ApiReturnType<A, ArchiveFile | undefined> {
        ...
        // For multi-archive indices (like sound effects), each archive contains one file
        // Archive ID = requested ID, file ID = 0
        const fileCount = this.getFileCount(id);
        if (fileCount >= 1) {
            return this.getFile(id, 0, key);
        }
        // Archive doesn't exist - return undefined instead of throwing
        return undefined as ApiReturnType<A, ArchiveFile | undefined>;
    }
}
...
export class LegacyCacheIndex extends CacheIndex {
    ...
    override getArchiveId(name: string): number {
        return this.archiveNameHashes.get(CacheNameHash.hashOld(name)) ?? -1;
    }
