...
import { TextureMaterial } from "@august/osrs-engine/texture/TextureMaterial";
import { ProceduralTexture } from "@august/osrs-engine/texture/procedural/ProceduralTexture";
import { TextureGenerator } from "@august/osrs-engine/texture/procedural/TextureGenerator";

/**
 * Procedural texture format whose material metadata is embedded with each
 * texture definition instead of living in a separate materials index.
 */
export class EmbeddedMaterialProceduralTextureLoader implements TextureLoader {
    ...
    static load(
    ...
    ): EmbeddedMaterialProceduralTextureLoader {
        ...
        return new EmbeddedMaterialProceduralTextureLoader(spriteIndex, textureIds, definitions);
    }
    ...
    getMaterial(id: number): TextureMaterial {
        ...
        return {
            ...
            alphaCutOff,
            frameCount: 1,
            animSpeed: Math.abs(speed),
        };
    }

    getFrameCount(_id: number): number {
        return 1;
    }
