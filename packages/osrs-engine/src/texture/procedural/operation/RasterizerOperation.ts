...
export class Rasterizer {
    ...
    static rasterHorizontalLine0(x0: number, x1: number, y0: number, color: number) {
        if (x1 >= x0) {
            RasterSpanFill.fillRange(Rasterizer.pixels[y0], x0, x1, color);
        } else {
            RasterSpanFill.fillRange(Rasterizer.pixels[y0], x1, x0, color);
        }
    }
    ...
    static rasterRectangle0(
        ...
        for (let local25 = y0; local25 < local6; local25++) {
            RasterSpanFill.fillRange(Rasterizer.pixels[local25], x0, x1, outlineColor);
        }
        for (let local55 = y1; local55 > local14; local55--) {
            RasterSpanFill.fillRange(Rasterizer.pixels[local55], x0, x1, outlineColor);
        }
        for (let local75 = local6; local75 <= local14; local75++) {
            const local86 = Rasterizer.pixels[local75];
            RasterSpanFill.fillRange(local86, x0, local18, outlineColor);
            RasterSpanFill.fillRange(local86, local18, local23, fillColor);
            RasterSpanFill.fillRange(local86, local23, x1, outlineColor);
        }
    }
    ...
    static rasterRectangleClamped(
        ...
        for (let local54 = local11; local54 < local43; local54++) {
            RasterSpanFill.fillRange(Rasterizer.pixels[local54], local28, local34, outlineColor);
        }
        for (let local74 = local22; local74 > local52; local74--) {
            RasterSpanFill.fillRange(Rasterizer.pixels[local74], local28, local34, outlineColor);
        }
        ...
        for (let local108 = local43; local108 <= local52; local108++) {
            ...
            RasterSpanFill.fillRange(local119, local28, local97, outlineColor);
            RasterSpanFill.fillRange(local119, local97, local106, fillColor);
            RasterSpanFill.fillRange(local119, local106, local34, outlineColor);
        }
    }
    ...
    static rasterRectangleFill0(x0: number, x1: number, y0: number, y1: number, fillColor: number) {
        for (let local6 = y0; local6 <= y1; local6++) {
            RasterSpanFill.fillRange(Rasterizer.pixels[local6], x0, x1, fillColor);
        }
    }
    ...
    static rasterRectangleFillClamped(
        ...
        for (let local31 = local11; local31 <= local17; local31++) {
            RasterSpanFill.fillRange(Rasterizer.pixels[local31], local23, local29, fillColor);
        }
    }
    ...
    static rasterRectangleOutlineWidth1(
        ...
        RasterSpanFill.fillRange(Rasterizer.pixels[y0++], x0, x1, outlineColor);
        RasterSpanFill.fillRange(Rasterizer.pixels[y1--], x0, x1, outlineColor);
    ...
    static rasterRectangleOutline0(
        ...
        for (let local24 = y0; local24 < local10; local24++) {
            RasterSpanFill.fillRange(Rasterizer.pixels[local24], x0, x1, outlineColor);
        }
        for (let local44 = y1; local44 > local18; local44--) {
            RasterSpanFill.fillRange(Rasterizer.pixels[local44], x0, x1, outlineColor);
        }
        const local66 = x1 - outlineWidth;
        for (let local68 = local10; local68 <= local18; local68++) {
            const local79 = Rasterizer.pixels[local68];
            RasterSpanFill.fillRange(local79, x0, local22, outlineColor);
            RasterSpanFill.fillRange(local79, local66, x1, outlineColor);
        }
    }
    ...
    static rasterRectangleOutlineClamped(
        ...
        for (let local54 = local17; local54 < local43; local54++) {
            RasterSpanFill.fillRange(Rasterizer.pixels[local54], local29, local35, outlineColor);
        }
        for (let local70 = local23; local70 > local52; local70--) {
            RasterSpanFill.fillRange(Rasterizer.pixels[local70], local29, local35, outlineColor);
        }
        const local97 = clamp(x0 + outlineWidth, Rasterizer.startX, Rasterizer.widthMask);
        const local106 = clamp(x1 - outlineWidth, Rasterizer.startX, Rasterizer.widthMask);
        for (let local108 = local43; local108 <= local52; local108++) {
            const local119 = Rasterizer.pixels[local108];
            RasterSpanFill.fillRange(local119, local29, local97, outlineColor);
            RasterSpanFill.fillRange(local119, local106, local35, outlineColor);
        }
    }
    ...
    static rasterEllipse0(
        ...
        RasterSpanFill.fillRange(local154, x - sizeX, x - local14, outlineColor);
        RasterSpanFill.fillRange(local154, x - local14, local14 + x, fillColor);
        RasterSpanFill.fillRange(local154, local14 + x, x + sizeX, outlineColor);
        while (local9 > 0) {
            if (local71 < 0) {
                while (local71 < 0) {
                    local71 += local118;
            ...
            if (local258) {
                ...
                RasterSpanFill.fillRange(Rasterizer.pixels[local352], local251, local358, outlineColor);
                const local371 = x + local16;
                RasterSpanFill.fillRange(Rasterizer.pixels[local352], local358, local371, fillColor);
                RasterSpanFill.fillRange(Rasterizer.pixels[local352], local371, local263, outlineColor);
                RasterSpanFill.fillRange(Rasterizer.pixels[local277], local251, local358, outlineColor);
                RasterSpanFill.fillRange(Rasterizer.pixels[local277], local358, local371, fillColor);
                RasterSpanFill.fillRange(Rasterizer.pixels[local277], local371, local263, outlineColor);
            } else {
                RasterSpanFill.fillRange(Rasterizer.pixels[local352], local251, local263, outlineColor);
                RasterSpanFill.fillRange(Rasterizer.pixels[local277], local251, local263, outlineColor);
            }
        }
    }
    ...
    static rasterEllipseClamped(
        ...
        if (y >= Rasterizer.startY && Rasterizer.heightMask >= y) {
            ...
            RasterSpanFill.fillRange(local166, local177, local193, outlineColor);
            RasterSpanFill.fillRange(local166, local193, local203, fillColor);
            RasterSpanFill.fillRange(local166, local203, local185, outlineColor);
        }
        while (local9 > 0) {
            if (local69 < 0) {
                while (local69 < 0) {
            ...
            if (local363 >= Rasterizer.startY && Rasterizer.heightMask >= local367) {
                ...
                if (local281) {
                    ...
                    if (Rasterizer.startY <= local367) {
                        ...
                        RasterSpanFill.fillRange(local426, local398, local418, outlineColor);
                        RasterSpanFill.fillRange(local426, local418, local409, fillColor);
                        RasterSpanFill.fillRange(local426, local409, local389, outlineColor);
                    }
                    if (Rasterizer.heightMask >= local363) {
                        const local456 = Rasterizer.pixels[local363];
                        RasterSpanFill.fillRange(local456, local398, local418, outlineColor);
                        RasterSpanFill.fillRange(local456, local418, local409, fillColor);
                        RasterSpanFill.fillRange(local456, local409, local389, outlineColor);
                    }
                } else {
                    if (local367 >= Rasterizer.startY) {
                        RasterSpanFill.fillRange(
    ...
    static rasterCircle0(
        ...
        RasterSpanFill.fillRange(local27, x - size, local39, outlineColor);
        const local57 = local17 + x;
        let local60 = -local17;
        RasterSpanFill.fillRange(local27, local39, local57, fillColor);
        RasterSpanFill.fillRange(local27, local57, x + size, outlineColor);
        while (local19 > local10) {
            ...
            if (local13 >= 0) {
                ...
                if (local19 >= local17) {
                    ...
                    RasterSpanFill.fillRange(local126, local143, local131, outlineColor);
                    RasterSpanFill.fillRange(local138, local143, local131, outlineColor);
                } else {
                    ...
                    RasterSpanFill.fillRange(local163, local191, local182, outlineColor);
                    RasterSpanFill.fillRange(local163, local182, local186, fillColor);
                    RasterSpanFill.fillRange(local163, local186, local178, outlineColor);
                    RasterSpanFill.fillRange(local174, local191, local182, outlineColor);
                    RasterSpanFill.fillRange(local174, local182, local186, fillColor);
                    RasterSpanFill.fillRange(local174, local186, local178, outlineColor);
                }
                local13 -= local19 << 1;
            }
            ...
            if (local17 <= local10) {
                RasterSpanFill.fillRange(local240, local256, local251, outlineColor);
                RasterSpanFill.fillRange(local247, local256, local251, outlineColor);
            } else {
                ...
                RasterSpanFill.fillRange(local240, local256, local294, outlineColor);
                RasterSpanFill.fillRange(local240, local294, local290, fillColor);
                RasterSpanFill.fillRange(local240, local290, local251, outlineColor);
                RasterSpanFill.fillRange(local247, local256, local294, outlineColor);
                RasterSpanFill.fillRange(local247, local294, local290, fillColor);
                RasterSpanFill.fillRange(local247, local290, local251, outlineColor);
            }
        }
    }
    ...
    static rasterCircleClamped(
        ...
        if (Rasterizer.startY <= y && y <= Rasterizer.heightMask) {
            ...
            RasterSpanFill.fillRange(local40, local48, local67, outlineColor);
            RasterSpanFill.fillRange(local40, local67, local77, fillColor);
            RasterSpanFill.fillRange(local40, local77, local56, outlineColor);
        }
        let local98 = -local9;
        let local100 = -1;
        let local102 = -1;
                ...
                if (Rasterizer.startY <= local159 && local154 <= Rasterizer.heightMask) {
                    if (local15 >= local9) {
                        ...
                        if (Rasterizer.heightMask >= local159) {
                            ...
                            RasterSpanFill.fillRange(local274, local245, local262, outlineColor);
                            RasterSpanFill.fillRange(local274, local262, local254, fillColor);
                            RasterSpanFill.fillRange(local274, local254, local237, outlineColor);
                        }
                        if (Rasterizer.startY <= local154) {
                            const local300 = Rasterizer.pixels[local154];
                            RasterSpanFill.fillRange(local300, local245, local262, outlineColor);
                            RasterSpanFill.fillRange(local300, local262, local254, fillColor);
                            RasterSpanFill.fillRange(local300, local254, local237, outlineColor);
                        }
                    }
                }
            }
                ...
                if (local337 >= Rasterizer.startX && Rasterizer.widthMask >= local342) {
                    ...
                    if (local17 >= local9) {
                        ...
                        if (Rasterizer.heightMask >= local322) {
                            ...
                            RasterSpanFill.fillRange(local440, local365, local432, outlineColor);
                            RasterSpanFill.fillRange(local440, local432, local424, fillColor);
                            RasterSpanFill.fillRange(local440, local424, local359, outlineColor);
                        }
                        if (Rasterizer.startY <= local327) {
                            const local469 = Rasterizer.pixels[local327];
                            RasterSpanFill.fillRange(local469, local365, local432, outlineColor);
                            RasterSpanFill.fillRange(local469, local432, local424, fillColor);
                            RasterSpanFill.fillRange(local469, local424, local359, outlineColor);
                        }
                    }
                }
            }
    ...
    static rasterEllipseFill0(
        ...
        RasterSpanFill.fillRange(Rasterizer.pixels[y], x - sizeX, sizeX + x, fillColor);
        let local20 = 0;
        let local22 = sizeY;
        ...
        while (local22 > 0) {
            ...
            RasterSpanFill.fillRange(Rasterizer.pixels[local150], local181, local159, fillColor);
            RasterSpanFill.fillRange(Rasterizer.pixels[local167], local181, local159, fillColor);
        }
    }
    ...
    static rasterEllipseFillClamped(
        ...
        if (y >= Rasterizer.startY && Rasterizer.heightMask >= y) {
            ...
            RasterSpanFill.fillRange(Rasterizer.pixels[y], local117, local109, fillColor);
        }
    ...
    static rasterCircleFill0(x: number, y: number, size: number, fillColor: number) {
        RasterSpanFill.fillRange(Rasterizer.pixels[y], x - size, size + x, fillColor);
        let local20 = 0;
        let local33 = size;
        let local36 = -size;
        let local38 = -1;
        while (local33 > local20) {
            ...
            if (local36 >= 0) {
                ...
                RasterSpanFill.fillRange(local76, local80, local84, fillColor);
                RasterSpanFill.fillRange(local69, local80, local84, fillColor);
            }
            ...
            RasterSpanFill.fillRange(local112, local106, local101, fillColor);
            RasterSpanFill.fillRange(local118, local106, local101, fillColor);
        }
    }
    ...
    static rasterCircleFillClamped(x: number, y: number, size: number, fillColor: number) {
        ...
        RasterSpanFill.fillRange(Rasterizer.pixels[y], local35, local27, fillColor);
