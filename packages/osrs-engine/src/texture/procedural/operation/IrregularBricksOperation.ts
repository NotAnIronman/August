import JavaRandom from "@august/game-model/math/JavaRandom";

import { nextIntJagex } from "@august/game-model/math/MathUtil";
                ...
                if (local93 <= local106) {
                    ...
                    if (local93 <= local260) {
                        ...
                        if (local97 + local104 <= textureGenerator.width) {
                            RasterSpanFill.fill(local116, local97, local104, local20);
                        } else {
                            const local461 = textureGenerator.width - local97;
                            RasterSpanFill.fill(local116, local97, local461, local20);
                            RasterSpanFill.fill(local116, 0, local104 - local461, local20);
                        }
                    } else {
                        ...
                        if (textureGenerator.width < local104 + local97) {
                            ...
                            RasterSpanFill.fill(local116, local97, local379, local274);
                            RasterSpanFill.fill(local116, 0, local104 - local379, local274);
                        } else {
                            RasterSpanFill.fill(local116, local97, local104, local274);
                        }
                    }
                } else {
                    ...
                    if (local97 + local104 > textureGenerator.width) {
                        ...
                        RasterSpanFill.fill(local116, local97, local231, local130);
                        RasterSpanFill.fill(local116, 0, local104 - local231, local130);
                    } else {
                        RasterSpanFill.fill(local116, local97, local104, local130);
                    }
                }
            }
        } else if (textureGenerator.width >= arg1 + arg2) {
            for (let i = 0; i < arg0; i++) {
                RasterSpanFill.fill(pixels[i + arg3], arg2, arg1, local20);
            }
        } else {
            const local507 = textureGenerator.width - arg2;
            for (let i = 0; i < arg0; i++) {
                const local518 = pixels[i + arg3];
                RasterSpanFill.fill(local518, arg2, local507, local20);
                RasterSpanFill.fill(local518, 0, arg1 - local507, local20);
            }
        }
    }
