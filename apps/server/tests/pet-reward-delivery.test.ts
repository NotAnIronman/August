...
const earned = setup();
const source = { bossNpcTypeId: 13668, bossName: "Araxxor", killcount: 17 };
earned.ground.spawn(petId, 1, tile, 10, { ownerId: 42, isMonsterDrop: true, petDropSource: source });
assert.equal(earned.player.followers.getFirstPetDrop(petId)?.killcount, 17);
awardPetReward(earned.player, petId, 1, earned.services, { ...source, killcount: 88 });
assert.equal(earned.player.followers.getFirstPetDrop(petId)?.killcount, 17);
const historical = setup();
historical.player.collectionLog.addItem(29837);
awardPetReward(historical.player, petId, 1, historical.services, source);
assert.equal(historical.player.followers.getFirstPetDrop(petId), undefined, "do not invent first KC for an older pet or its variant");
