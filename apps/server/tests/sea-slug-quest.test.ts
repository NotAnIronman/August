...
import groundItemSpawns from "@august/data/catalogs/server/ground-item-spawns.json";
...
const services = {
    ...
    inventory: {
        ...
        addItemToInventory: (_player: unknown, itemId: number, quantity: number) => {
            ...
            if (
                existing &&
                ([ITEM.swampTar, ITEM.rawSwampPaste, ITEM.swampPaste] as readonly number[]).includes(
                    itemId,
                )
            ) {
