 ...
 *
 * Run with: pnpm exec tsx tests/instance-parity.test.ts
 */
...
import { clientPath, repositoryPath, serverPath } from "@server/paths";
...
describe("REBUILD_NORMAL packet", () => {
    it("has a ServerMessageId", () => {
        assertEqual(ServerMessageId.REBUILD_NORMAL, 141, "packet id");
    });
    it("has variable short length (-2)", () => {
        assertEqual(SERVER_MESSAGE_LENGTHS[ServerMessageId.REBUILD_NORMAL], -2, "packet length");
    });
    it("REBUILD_REGION is still registered", () => {
        assertEqual(ServerMessageId.REBUILD_REGION, 140, "rebuild_region id");
        assertEqual(
            SERVER_MESSAGE_LENGTHS[ServerMessageId.REBUILD_REGION],
            -2,
...
describe("clearTerrainChunk parity", () => {
    it("reference only clears tileHeights, not tileRenderFlags", () => {
        ...
        const source = readFileSync(
            repositoryPath("packages", "osrs-engine", "src", "scene", "SceneBuilder.ts"),
            "utf-8",
        );
...
describe("REBUILD_NORMAL client dispatch", () => {
    it("inbound sync handler handles rebuild_normal message", () => {
        const source = readFileSync(
            clientPath("src", "core", "network", "server-connection", "handlers", "inboundSync.ts"),
            "utf-8",
        );
    ...
    it("OsrsClient subscribes to rebuildNormal", () => {
        const source = readFileSync(clientPath("src", "engine", "game", "OsrsClient.ts"), "utf-8");
    ...
    it("WebGLOsrsRenderer has clearInstance method", () => {
        const source = readFileSync(
            clientPath("src", "engine", "rendering", "WebGLOsrsRenderer.ts"),
            "utf-8",
        );
...
describe("Server-side REBUILD_NORMAL", () => {
    it("ServerBinaryEncoder has encodeRebuildNormal", () => {
        const source = readFileSync(
            serverPath("src", "network", "packet", "ServerBinaryEncoder.ts"),
            "utf-8",
        );

        assert(source.includes("encodeRebuildNormal("), "encodeRebuildNormal exists");
        assert(source.includes("ServerMessageId.REBUILD_NORMAL"), "uses REBUILD_NORMAL packet ID");
    });

    it("messages.ts routes rebuild_normal", () => {
        const source = readFileSync(serverPath("src", "network", "messages.ts"), "utf-8");
    ...
    it("WorldEntityService has sendRebuildNormal method", () => {
        const source = readFileSync(
            serverPath("src", "game", "services", "WorldEntityService.ts"),
            "utf-8",
        );
...
describe("ServerBinaryDecoder REBUILD_NORMAL", () => {
    it("decoder case exists", () => {
        const source = readFileSync(
            clientPath("src", "core", "network", "packet", "ServerBinaryDecoder.ts"),
            "utf-8",
        );

        assert(source.includes("ServerMessageId.REBUILD_NORMAL"), "decoder handles REBUILD_NORMAL");
