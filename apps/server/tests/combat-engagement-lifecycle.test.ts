...
import { createTestPathService } from "./fixtures/createTestPathService";
import { NpcState } from "@server/game/npc";
import { PlayerState } from "@server/game/player";

const TEST_GAMEMODE = createTestGamemode("combat-lifecycle-test", "Combat lifecycle test");
...
const engine = new CombatTickEngine({
    pathService: createTestPathService(),
...
const stunnedEngine = new CombatTickEngine({
    pathService: createTestPathService({ edgeHasWallBetween: () => false }),
    getPlayer: (id) => (id === player.id ? player : undefined),
