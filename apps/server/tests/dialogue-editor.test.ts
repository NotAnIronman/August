...
import { registerDevDialogueEditor, openDialogueEditorForNpc } from "@server/content/gamemodes/vanilla/widgets/devDialogueEditor";
import type { IScriptRegistry, ScriptServices, CommandHandler } from "@server/game/scripts/types";
import type { CommandRegistrationMetadata } from "@server/game/commands/CommandMetadata";
import type { DialogueStep, DialogueTreeJson } from "@server/game/dialogue/DialogueTree";
import { validateDialogueTreeJson } from "@server/game/dialogue/DialogueTree";
import { ComponentIds } from "@august/protocol/uikit/contracts";

const commands = new Map<string, CommandHandler>();
const commandMetadata = new Map<string, CommandRegistrationMetadata>();
...
const storesByNpc = new Map<number, DialogueStep[]>();
...
for (const [name, metadata] of commandMetadata) {
    assert.equal(metadata.permission, "developer", `${name} must remain developer-only`);
    assert.equal(metadata.owner, "developer:dialogue-editor");
    assert.equal(metadata.hidden, true, `${name} is protocol plumbing, not help-list content`);
}
