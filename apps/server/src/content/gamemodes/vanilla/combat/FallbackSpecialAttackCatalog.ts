/**
 * Vanilla fallback special-attack catalog.
 *
 * Compatibility definitions used only when no item-specific script or combat
 * profile handles the weapon. New special attacks belong in the core
 * special-attacks/implementations boundary.
 ...
 */
import type {
    FallbackSpecialAttackDefinition,
    FallbackSpecialAttackProvider,
...
class FallbackSpecialAttackCatalogImpl implements FallbackSpecialAttackProvider {
    private readonly specs = new Map<number, FallbackSpecialAttackDefinition>();
    ...
    private register(def: FallbackSpecialAttackDefinition): void {
...
export function createFallbackSpecialAttackProvider(): FallbackSpecialAttackProvider {
    return new FallbackSpecialAttackCatalogImpl();
}
