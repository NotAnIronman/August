...
import { findTraversalOverride } from "@server/content/gamemodes/vanilla/scripts/content/traversal";
...
//   3. Intermap link table – loaded from data/generated/server/intermap-links.json
...
function loadIntermapLinks(): IntermapLinkIndex {
    const filePath = serverGeneratedDataPath("intermap-links.json");
...
function loadStairFloors(): Map<number, StairFloorEntry> {
    const filePath = serverGeneratedDataPath("stair-floors.json");
    try {
        const raw = fs.readFileSync(filePath, "utf-8");
