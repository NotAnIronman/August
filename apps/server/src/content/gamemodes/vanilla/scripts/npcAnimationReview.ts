...
import { NPC_ANIMATION_REVIEW_PANEL_GROUP_ID } from "@august/protocol/ui/widgets/custom/journalPanel.cs2";
import { ComponentIds, type UiMenuButton } from "@august/protocol/uikit/contracts";
...
function resolveDataPath(fileName: string): string {
    if (fileName === "npc-animation-candidates.json") {
        return generatedReportPath("npc-animations", fileName);
    }
    if (fileName.startsWith("reports/")) {
        return generatedReportPath("npc-animations", fileName.slice("reports/".length));
    }
    return serverGeneratedDataPath(fileName);
}
...
export function registerNpcAnimationReviewCommands(
    ...
    registry.registerCommand("npcreview", ({ player, args }) => {
        ...
        owner: "developer:npc-animation-review",
        summary: "Review and classify NPC animation sequences.",
    });
