...
function assemble({ player, services }: { player: import("@server/game/player").PlayerState; services: ScriptServices }): void {
    ...
    const exchange = applyInventoryTransform(services.inventory, player, {
        inputs: KEY_PIECES.map((itemId) => ({ itemId, quantity: 1 })),
        outputs: [{ itemId: FROZEN_KEY, quantity: 1 }],
...
function openDoor({ player, services }: LocInteractionEvent): void {
    ...
    const consumed = applyInventoryTransform(services.inventory, player, {
        inputs: [{ itemId: FROZEN_KEY, quantity: 1 }],
        outputs: [],
    });
