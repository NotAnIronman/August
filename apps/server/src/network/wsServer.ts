...
import { ArchiveHealthBarDefinitionLoader } from "@august/osrs-engine/config/healthbar/HealthBarDefinitionLoader";
...
import { ACCOUNT_SUMMARY_GROUP_ID } from "@august/protocol/ui/accountSummary";
import { MUSIC_GROUP_ID } from "@august/protocol/ui/music";
import { VARP_FOLLOWER_INDEX } from "@august/game-model/state/vars";
import { MusicCatalogService } from "@server/audio/MusicCatalogService";
...
import { loadCollectionLogItems } from "@server/game/collectionlog";
import { DialogueActionRegistry } from "@server/game/dialogue/DialogueActionRegistry";
import { DialogueOverrideStore } from "@server/game/dialogue/DialogueOverrideStore";
import { runDialogueTree } from "@server/game/dialogue/DialogueTreeRunner";
...
import { CombatCategoryData } from "@server/game/combat/CombatCategoryData";
...
import { CollectionLogService } from "@server/game/services/CollectionLogService";
import { CombatDataService } from "@server/game/services/CombatDataService";
import { CombatEffectService } from "@server/game/services/CombatEffectService";
import { DataLoaderService } from "@server/game/services/DataLoaderService";
import { EquipmentService } from "@server/game/services/EquipmentService";
import { EquipmentStatsUiService } from "@server/game/services/EquipmentStatsUiService";
import { FriendsChatService } from "@server/game/services/FriendsChatService";
import { buildGamemodeServices } from "@server/game/services/GamemodeServiceAdapter";
import { InterfaceManager as ExtractedInterfaceManager } from "@server/game/services/InterfaceManager";
...
import { LocationService } from "@server/game/services/LocationService";
import { MessagingService } from "@server/game/services/MessagingService";
import { MovementService } from "@server/game/services/MovementService";
import { PlayerCombatService } from "@server/game/services/PlayerCombatService";
import {
    ProjectileTimingService,
    type ProjectileTimingServiceDeps,
} from "@server/game/services/ProjectileTimingService";
import {
    type ScriptServiceAdapterDeps,
    buildScriptServices,
} from "@server/game/services/ScriptServiceAdapter";
...
import { InterfaceService } from "@server/widgets/InterfaceService";
import type { WidgetAction } from "@server/widgets/WidgetManager";
import { registerCollectionLogInterfaceHooks } from "@server/widgets/hooks/CollectionLogInterfaceHooks";
import { registerDialogInterfaceHooks } from "@server/widgets/hooks/DialogInterfaceHooks";
import { type CacheEnv, initCacheEnv } from "@server/world/CacheEnv";
...
import { DoorDefinitionLoader } from "@server/world/DoorDefinitionLoader";
import { DoorRuntimeTileMappingStore } from "@server/world/DoorRuntimeTileMappingStore";
import { DoorStateManager } from "@server/world/DoorStateManager";
import { DynamicLocStateStore } from "@server/world/DynamicLocStateStore";
import { LocTileLookupService } from "@server/world/LocTileLookupService";
import { locCanResolveToId } from "@server/world/LocTransforms";
...
import { NpcPacketEncoder, PlayerPacketEncoder } from "@server/network/encoding";
import { encodeAppearanceBinary } from "@server/network/encoding/AppearanceEncoder";
import { WorldEntityInfoEncoder } from "@server/network/encoding/WorldEntityInfoEncoder";
import {
    Cs2ModalManager,
    GroundItemHandler,
    NpcSyncManager,
...
import { REPORT_GAME_TIME_GROUP_ID, ReportGameTimeTracker } from "@server/network/reportGameTime";
import type { NpcUpdatePayload, NpcViewSnapshot, TickFrame } from "@server/network/wsServerTypes";
import {
    DEFAULT_AUTOSAVE_SECONDS,
    GROUND_ITEM_DESPAWN_TICKS,
    GROUND_ITEM_PRIVATE_TICKS,
} from "@server/network/wsServerTypes";
...
export class WSServer {
    ...
    private initBroadcasters(): void {
        this.playerPersistence = new PlayerPersistence({
            ...
            defaultsPath: getGamemodeDefaultsPath(this.gamemode.id),
        });
    ...
    private initCacheEnvironment(opts: WSServerOptions): void {
        const env = opts.cacheEnv ?? initCacheEnv(serverVarPath("cache", "osrs"));
        this.cacheEnv = env;
    ...
    private initDoorSystem(opts: WSServerOptions): void {
        ...
        if (locTypeLoader) {
            ...
            // Loc/npc map building is now handled by content modules during register().
        }
    }
    ...
    private initDeferredDeps(opts: WSServerOptions): void {
        ...
        if (this.cacheFactory) {
            ...
            let enumTypeLoader:
                | import("@august/osrs-engine/config/enumtype/EnumTypeLoader").EnumTypeLoader
                | undefined;
            try {
    ...
    private serializeAppearancePayload(
        view: import("@server/network/encoding/types").PlayerViewSnapshot,
    ): Uint8Array {
