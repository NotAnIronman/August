...
} from "@server/world/InstanceManager";
import { sendMessage, serverEncoder } from "@server/network/packet/BinaryProtocol";
...
export type CollectionLogServerPayload =
    ...
    | {
           ...
           *  client/network/server-connection/types/messages.ts - they're
...
export function decodeClientMessage(raw: string | Buffer | ArrayBuffer): RoutedMessage | null {
    ...
    if (raw instanceof Buffer || raw instanceof ArrayBuffer) {
        ...
        } = require("@server/network/packet/ClientBinaryDecoder");
