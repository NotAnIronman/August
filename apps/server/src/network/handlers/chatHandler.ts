import { VARBIT_ACTIVE_SPELLBOOK, VARP_SPECIAL_ENERGY } from "@august/game-model/state/vars";
import {
    MAX_REAL_LEVEL,
    SKILL_IDS,
    SkillId,
...
} from "@server/game/dev/DeveloperFlags";
import type { PlayerState } from "@server/game/player";
import type { BankingServices } from "@server/game/scripts/serviceInterfaces";
...
import { PLAYER_PERMISSIONS, type PlayerPermission, hasPermission } from "@server/network/PlayerPermission";
import {
    ...
    return (ctx) => {
        try {
            ...
            if (text.startsWith("::")) {
                ...
                const registeredCommand = services.findRegisteredCommand(root);
                const resolvedCommand = resolveCommand(root, registeredCommand);
                if (!resolvedCommand) return;
                const requiredPermission = resolvedCommand.metadata.permission;
                if (!hasPermission(services.getPlayerPermission(sender), requiredPermission)) {
                    services.queueChatMessage({
                        messageType: "game",
                ...
                // Content-owned handlers execute only after unified resolution
                // and permission checks above.
                if (resolvedCommand.source === "content") {
                    try {
                        const result = resolvedCommand.handler({
                            player: sender,
                            command: root,
                            args: originalCaseParts.slice(1),
                            tick: services.getCurrentTick(),
                            services: services as unknown as ScriptServices,
                        });
