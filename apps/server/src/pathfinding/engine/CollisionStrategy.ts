...
class NormalCollisionStrategy implements CollisionStrategy {
    canMove(tileFlag: number, blockFlag: number): boolean {
        // Vanilla: tiles flagged with FLOOR (from tileRenderFlags bit 0x1)
        // are blocked for normal movement. Only the BLOCKED strategy
        // allows movement constrained to such tiles.
