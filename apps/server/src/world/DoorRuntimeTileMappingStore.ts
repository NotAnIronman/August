...
import { serverCatalogPath, serverVarPath } from "@server/paths";
...
} from "@server/world/DoorCatalogFile";
...
export class DoorRuntimeTileMappingStore {
    ...
    constructor(
        private readonly filePath: string = serverVarPath("world", "door-tile-mappings.json"),
        private readonly flushDelayMs: number = 2000,
        private readonly seedPath: string = serverCatalogPath("doors.json"),
    ) {
        this.loadFile(this.seedPath);
        if (path.resolve(this.seedPath) !== path.resolve(this.filePath)) {
    ...
    private loadFile(filePath: string): void {
        ...
        try {
            const parsed = readRuntimeTileMappingsFromFile(filePath);
            ...
            logger.info(
                `[DoorRuntimeTileMappingStore] Loaded ${this.pairsByTile.size} tile mapping(s) from ${filePath}`,
            );
        } catch (err) {
            logger.warn(`[DoorRuntimeTileMappingStore] Failed to load ${filePath}`, err);
        }
    }
