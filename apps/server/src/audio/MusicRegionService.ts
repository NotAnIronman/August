...
function loadMusicData(): MusicData {
    try {
        return JSON.parse(fs.readFileSync(serverCatalogPath("music-regions.json"), "utf-8"));
    } catch {
        logger.warn("[MusicRegionService] Could not load data/catalogs/server/music-regions.json");
