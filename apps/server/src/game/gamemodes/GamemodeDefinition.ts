...
import type { StaticGroundItemSpawn } from "@server/game/items/GroundItemManager";
...
export interface GamemodeDefinition {
    ...
    getDropTable?(npcTypeId: number): import("@server/game/drops/types").NpcDropTableDefinition | undefined;
    /** Provide additional drops beyond the base table. */
    getSupplementalDrops?(
        npcTypeId: number,
        player: PlayerState,
    ): import("@server/game/drops/types").NpcDropEntryDefinition[];
    /** Provide per-NPC loot distribution config (highest-damage, shared, etc.). */
    getLootDistributionConfig?(
        npcTypeId: number,
    ): import("@server/game/combat/DamageTracker").NpcLootConfig | undefined;
    /**
    ...
    getPlayerTypes(
        player: PlayerState,
        isAdmin: boolean,
    ): import("@august/osrs-engine/chat/PlayerType").PlayerType[];
    ...
    // === Content Data ===
    /** Controls boot-time loading of data/generated/server/npc-spawns.json. */
