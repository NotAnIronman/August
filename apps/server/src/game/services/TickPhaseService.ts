...
import type { PlayerTickFrameData } from "@server/network/encoding";
...
import {
    ...
    isInWilderness,
    multiCombatSystem,
} from "@server/game/combat/MultiCombatZones";
...
import { hasHitpointsCapeRegenPerk } from "@server/game/equipment";
import { deriveInteractionIndex } from "@server/game/interactions/InteractionViewBuilder";
import { NpcState, type NpcUpdateDelta } from "@server/game/npc";
...
import { EQUIPMENT_STATS_GROUP_ID } from "@server/game/services/EquipmentStatsUiService";
    ...
    private refreshInteractionFacing(frame: TickFrame): void {
        ...
        players.forEachBot((bot) => {
            const interactionState = (
                bot as PlayerState & {
                    botInteraction?: import("@server/game/interactions/types").PlayerInteractionState;
                }
            ).botInteraction;
            collectFaceTile(bot);
            const interactionIndex = deriveInteractionIndex({
