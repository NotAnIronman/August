 ...
 * - handleFallbackSpecialAttackEffects (special attack effects)
...
import type { PoweredStaffSpellData } from "@server/game/spells/SpellDataProvider";
import type {
...
import type { CombatActionServices, SpecialAttackPayload } from "@server/game/actions/handlers/CombatActionHandler";
...
export class NpcHitHandler {
    ...
    executeCombatPlayerHitAction(
        ...
        const incomingDamageMultiplier = data.twinflameEcho && npc.incomingPlayerDamageMultiplier>0 ? 1 : Math.max(0, npc.incomingPlayerDamageMultiplier);
        ...
        queueTwinflameActionEcho(this.services,player.id,data,npcHitsplat.amount,npcHitsplat.hpCurrent,tick);
        ...
        this.handleFallbackSpecialAttackEffects(player, npc, data, hitLanded, npcHitsplat, tick);
    ...
    handleFallbackSpecialAttackEffects(
        player: PlayerState,
        npc: NpcState,
        data: CombatPlayerHitActionData,
        landed: boolean,
