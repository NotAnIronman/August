...
import { resolvePlayerAttackType } from "@server/game/combat/CombatRules";
import {
    DamageType,
    damageTracker,
    type DropEligibility,
} from "@server/game/combat/DamageTracker";
import { HITMARK_BLOCK, HITMARK_DAMAGE } from "@server/game/combat/HitEffects";
import { isInWilderness, multiCombatSystem } from "@server/game/combat/MultiCombatZones";
import { CombatAttributes } from "@server/game/combat/state/CombatAttributes";
import { getFallbackSpecialAttack } from "@server/game/combat/special-attacks/FallbackSpecialAttackProvider";
import { pickSpecialAttackVisualOverride } from "@server/game/combat/special-attacks/SpecialAttackVisualProvider";
import { getSpellBaseXp } from "@server/game/combat/SpellXpProvider";
import { getRangedImpactSound } from "@server/game/combat/WeaponDataProvider";
import { getProjectileParams, type ProjectileLifeModel } from "@server/game/data/ProjectileParamsProvider";
import { consumeEquippedAmmoApply, ensureEquipQtyArrayOn } from "@server/game/equipment";
import type { NpcState } from "@server/game/npc";
import type { PendingNpcDrop } from "@server/game/npcManager";
import type { PlayerAppearance, PlayerState, SkillSyncUpdate } from "@server/game/player";
import type { SpellCastContext, SpellCastOutcome } from "@server/game/spells/SpellCaster";
import { SpellCaster } from "@server/game/spells/SpellCaster";
import type { PoweredStaffSpellData, SpellDataEntry } from "@server/game/spells/SpellDataProvider";
import {
...
} from "@server/game/actions/actionPayloads";
import type { ActionEffect, ActionExecutionResult, ActionRequest, ScheduledAction } from "@server/game/actions/types";
import { handleAutocastRuneConsumption, handleRangedAmmoConsumption } from "@server/game/actions/handlers/CombatHandlerUtils";
import { CompanionHitHandler } from "@server/game/actions/handlers/CompanionHitHandler";
import { NpcHitHandler } from "@server/game/actions/handlers/NpcHitHandler";
import { PvpCombatHandler } from "@server/game/actions/handlers/PvpCombatHandler";
import type { SpellResultPayload } from "@server/game/actions/handlers/SpellActionHandler";
...
export interface CombatActionServices {
    ...
    // --- Special Attacks ---
    /** Get the compatibility definition when no detailed special handles the weapon. */
    getFallbackSpecialAttack(weaponId: number): { soundId?: number } | undefined;
        ...
        return {
            ...
            processSpellCastRequest: (player, request) =>
                svc.spellActionHandler!.processSpellCastRequest(
                    player,
                    request as unknown as import("@server/game/actions/handlers/SpellActionHandler").SpellCastRequest,
            ...
            encodeMessage: (msg) =>
                encodeMessage(msg as unknown as import("@server/network/messages").ServerToClient),
            ...
            getFallbackSpecialAttack: (weaponId) => getFallbackSpecialAttack(weaponId),
            ...
            getAutocastCompatibilityMessage: (reason) =>
                getAutocastCompatibilityMessage(
                    reason as import("@server/game/spells/SpellDataProvider").AutocastCompatibilityResult["reason"],
                ),
    ...
    executeCombatAttackAction(
        ...
        if (!data.magicAutocastHandled) {
            ...
            if (specialActivated && weaponItemId > 0) {
                const specDef = getFallbackSpecialAttack(weaponItemId);
        ...
        const spawnProjectile = (baseProjectile: ProjectileParams): void => {
            ...
            const timing = this.svc.projectileTimingService!.estimateProjectileTiming({
                ...
                targetY: npc.tileY,
                projectileDefaults:
                    projectileToSpawn as import("@server/game/data/ProjectileParamsProvider").ProjectileParams,
                pathService,
            });
