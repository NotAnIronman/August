...
```text
pnpm --filter @august/server import-wiki-dialogue -- --page Man --section "Standard dialogue"
```

Generate a throttled batch:

```text
pnpm --filter @august/server import-wiki-dialogue -- --all --limit 100
```

`--all` walks the Wiki's `NPC dialogue` category rather than guessing NPCs
from page names.

Drafts are written to `data/catalogs/server/dialogue-imports`. They retain the source
