import { VARP_SPECIAL_ATTACK, VARP_SPECIAL_ENERGY } from "@august/game-model/state/vars";
...
import { DamageType, damageTracker } from "@server/game/combat/DamageTracker";
import {
...
import { getNpcPoisonConfig, hasVenomImmunityEquipment } from "@server/game/combat/PoisonVenomSystem";
...
} from "@server/game/combat/special-attacks/implementations/AncientGodswordSpec";
import {
    ARKAN_BLADE_BURN_PROFILE_ID,
    createArkanBladeBurnAttack,
    takeDueArkanBladeBurns,
} from "@server/game/combat/special-attacks/implementations/ArkanBladeSpec";
import { takeDueBurningClawBurns } from "@server/game/combat/special-attacks/implementations/BurningClawsSpec";
import {
...
import { clearNoxiousHalberdVirulenceOnWeaponChange } from "@server/game/combat/special-attacks/implementations/NoxiousHalberdSpec";
import {
...
import { consumeSoulflameHornEntice } from "@server/game/combat/special-attacks/implementations/SoulflameHornSpec";
import {
...
} from "@server/game/combat/engine/CombatHitEvaluator";
import { resolveNpcAttackAnimation } from "@server/game/combat/engine/NpcAttackAnimationResolver";
import type { CombatRetaliationEngine } from "@server/game/combat/engine/CombatRetaliationEngine";
...
export class CombatHitProcessor {
    ...
    constructor(
        ...
        WeaponSpecialAttackRegistry.initialize();
    ...
    processPreparedAttacks(
        ...
        for (const attack of attacks) {
            ...
            queuedHits += this.queueProfileTargetingHits(
                attack,
                attacker,
                target,
                profile,
    ...
    private queueProfileTargetingHits(
        ...
        const largeTargetHits = targeting.largeTargetExtraHits ??
            (targeting.largeTargetExtraHit ? [targeting.largeTargetExtraHit] : []);
        const targetSize = Math.max(1, Math.trunc(primaryTarget.size));
        const applicableLargeTargetHits = largeTargetHits.filter(
            (hit) => targetSize >= Math.max(2, Math.trunc(hit.minimumSize)),
        );
        if (applicableLargeTargetHits.length > 0) {
            let queuedHits = 0;
            for (const hit of applicableLargeTargetHits) {
                const accuracyMultiplier =
                    Math.max(0, specialAttack.accuracyMultiplier) *
                    Math.max(0, hit.accuracyMultiplier);
                queuedHits += this.queueSpecialSecondaryHit(
                    ...
                    specialAttack,
                    accuracyMultiplier,
                    Math.max(0, hit.damageMultiplier ?? 1),
                    currentMapClock,
                    travelDelayTicks,
                );
            }
    ...
    private resolveSpecialAttack(
        ...
        const script = weaponId === undefined ? undefined : WeaponSpecialAttackRegistry.get(weaponId);
        ...
        const legacySpecial =
            !script && !profileSpecial && weaponId !== undefined
                ? getFallbackSpecialAttack(weaponId)
                : undefined;
    ...
    private applyWeaponSpecialAttackScript(hit: AppliedCombatHit): void {
        ...
        const script = WeaponSpecialAttackRegistry.get(weaponId);
        if (!script) return;
