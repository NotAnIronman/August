...
export class ScriptRegistry implements IScriptRegistry {
    ...
    private readonly commandHandlers = new Map<string, RegisteredCommand<CommandHandler>>();
    ...
    registerCommand(
        ...
        registrationMetadata: CommandRegistrationMetadata,
    ): ScriptRegistrationResult {
        const metadata = createCommandMetadata(name, registrationMetadata);
        const normalized = metadata.name;
        warnOverwrite(this.commandHandlers, normalized, "command");
        const registration: RegisteredCommand<CommandHandler> = { metadata, handler };
        this.commandHandlers.set(normalized, registration);
        return {
            unregister: () => {
                if (this.commandHandlers.get(normalized) === registration) {
    ...
    findCommand(name: string): CommandHandler | undefined {
        return this.findCommandRegistration(name)?.handler;
    }
