 ...
 */
import { OSRS_CLIENT_PACKET_LENGTHS, OsrsClientPacketId } from "@client/core/network/packet/OsrsClientPacket";
...
export class PacketBufferNode {
    ...
    constructor(packetType: OsrsClientPacketId, bufferSize: number = 5000) {
        this.packetType = packetType;
        this.fixedLength = OSRS_CLIENT_PACKET_LENGTHS[packetType] ?? -1;
     ...
     */
    reset(packetType: OsrsClientPacketId): void {
        this.packetType = packetType;
        this.fixedLength = OSRS_CLIENT_PACKET_LENGTHS[packetType] ?? -1;
        this.index = 0;
...
class PacketBufferNodePool {
    ...
    acquire(packetType: OsrsClientPacketId): PacketBufferNode {
...
export class PacketWriter {
     ...
     */
    createPacket(packetType: OsrsClientPacketId): PacketBufferNode {
 ...
 */
export function createPacket(packetType: OsrsClientPacketId): PacketBufferNode {
