import type { WidgetActionClientPayload } from "@client/core/network/server-connection/types/index";
...
export function sendWidgetAction(payload: WidgetActionClientPayload): void {
    ...
    const { createPacket, queuePacket } = require("@client/core/network/packet/index");
    const { OsrsClientPacketId } = require("@august/protocol/transport/osrs/OsrsClientPacket");
    ...
    if (typeof normalized.subOpId === "number" && normalized.subOpId >= 1) {
        ...
        const pkt = createPacket(OsrsClientPacketId.IF_BUTTON_SUB);
 ...
 * Payload format mirrors OSRS `IF_TRIGGEROPLOCAL` (packet ID 30, var-short).
 */
export function sendIfTriggerOpLocal(
    ...
    // Import packet functions dynamically to avoid circular dependencies
    const { createPacket, queuePacket } = require("@client/core/network/packet/index");
    const { OsrsClientPacketId } = require("@client/core/network/packet/OsrsClientPacket");

    const pkt = createPacket(OsrsClientPacketId.IF_TRIGGEROPLOCAL);
...
export function sendPlayerDesignConfirm(appearance: {
    ...
    const pkt = createPacket(OsrsClientPacketId.APPEARANCE_SET);
