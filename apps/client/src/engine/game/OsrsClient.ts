...
import type { NpcInstance } from "@client/engine/rendering/npc/NpcRenderTemplate";
import { MenuTargetType, type OsrsMenuEntry } from "@august/osrs-engine/MenuEntry";
import { SoundEffectLoader } from "@august/osrs-engine/audio/SoundEffectLoader";
...
import { CacheLoaderFactory, getCacheLoaderFactory } from "@august/custom-content/items/cacheLoaderDecorator";
import { getPlayerTypeInfo } from "@august/osrs-engine/chat/PlayerType";
import { BasTypeLoader } from "@august/osrs-engine/config/bastype/BasTypeLoader";
import { DbRepository } from "@august/osrs-engine/config/db/DbRepository";
...
import { ObjModelLoader } from "@august/osrs-engine/config/objtype/ObjModelLoader";
import { ObjTypeLoader } from "@august/osrs-engine/config/objtype/ObjTypeLoader";
import { EquipToDisplaySlot, EquipmentSlot } from "@august/osrs-engine/config/player/Equipment";
import { PlayerAppearance } from "@august/osrs-engine/config/player/PlayerAppearance";
import type { SeqSoundEffect, SeqType } from "@august/osrs-engine/config/seqtype/SeqType";
...
import { Cs2Vm, ScriptArgMagic, type ScriptEvent, createScriptEvent } from "@client/engine/cs2/Cs2Vm";
import { Opcodes as Cs2Opcodes } from "@client/engine/cs2/Opcodes";
import { BitmapFont } from "@august/osrs-engine/font/BitmapFont";
import { encodeInteractionIndex } from "@august/osrs-engine/interaction/InteractionIndex";
import { Inventory, InventorySlotInput } from "@august/osrs-engine/inventory/Inventory";
import type { InventorySlot } from "@august/osrs-engine/inventory/Inventory";
import {
    MapFileIndex,
    getMapIndexFromTile,
    getMapPlaneId,
    getMapSquareId,
} from "@august/osrs-engine/map/MapFileIndex";
import { WorldMapState } from "@august/osrs-engine/map/WorldMapArea";
...
import { getOsrsInterfaceScalingPercent, setOsrsInterfaceScalingPercent } from "@client/ui/runtime/UiScale";
import {
    setNpcExamineIdResolver,
    setSpellSelectionClearHandler,
    setSpellSelectionResolver,
} from "@client/ui/runtime/menu/MenuAction";
import { type SimpleMenuEntry } from "@client/ui/runtime/menu/MenuEngine";
import { MenuOpcode, MenuState } from "@client/ui/runtime/menu/MenuState";
import { isDropTarget, isWidgetUseTarget } from "@client/ui/widgets/WidgetFlags";
...
import { WidgetSessionManager } from "@client/ui/widgets/WidgetSessionManager";
import { applyCollectionLogCompletionColors } from "@client/ui/widgets/custom/collectionLogCompletion";
import { applyQuestListWidgetGroups } from "@client/ui/widgets/custom/questList";
import { cleanupInterfaceClickTargets } from "@client/ui/widgets/gl/widgets-gl/index";
import { layoutWidgets } from "@client/ui/widgets/layout/WidgetLayout";
import { sanitizeText } from "@client/ui/widgets/menu/WidgetInteractionResolver";
...
import { CacheList, LoadedCache, getSparsePersistence } from "@client/core/cache/Caches";
import { Camera, CameraView, ProjectionType } from "@client/engine/rendering/camera/Camera";
import {
    ClientState,
...
import { resolveNpcEcsMapSquare } from "@client/engine/game/npc/NpcEcsMapSquare";
import { GameRenderer } from "@client/engine/rendering/core/GameRenderer";
import { OsrsRendererType, createRenderer } from "@client/engine/rendering/core/GameRenderers";
...
} from "@client/engine/game/TransmitCycles";
import { AudioVarpController } from "@client/engine/audio/AudioVarpController";
import { MusicSystem } from "@client/engine/audio/MusicSystem";
import { type SequenceSoundContext, SoundEffectSystem } from "@client/engine/audio/SoundEffectSystem";
import { ChatTextMetrics } from "@client/features/chat/ChatTextMetrics";
...
import { PlayerInteractionSystem } from "@client/engine/game/interactions/PlayerInteractionSystem";
import { IProjectileManager } from "@client/engine/game/interfaces/IProjectileManager";
import {
    GameState,
    type LoginAction,
    LoginErrorCode,
...
} from "@client/features/login/index";
import { NpcMovementSync } from "@client/engine/game/movement/NpcMovementSync";
import { PlayerMovementSync } from "@client/engine/game/movement/PlayerMovementSync";
import { NpcInstanceFlushController } from "@client/engine/game/npc/NpcInstanceFlushController";
import { createBrowserGroundItemsPluginPersistence } from "@client/features/plugins/grounditems/BrowserGroundItemsPluginPersistence";
import { GroundItemsPlugin } from "@client/features/plugins/grounditems/GroundItemsPlugin";
import { createBrowserInteractHighlightPluginPersistence } from "@client/features/plugins/interacthighlight/BrowserInteractHighlightPluginPersistence";
...
} from "@client/engine/game/selectedSpellPackets";
import { createBrowserSidebarPersistence } from "@client/features/sidebar/BrowserSidebarPersistence";
import { SidebarStore } from "@client/features/sidebar/SidebarStore";
import {
    type ClientSidebarEntryData,
    type SidebarPluginVisibilityOptions,
    registerDefaultClientSidebarEntries,
} from "@client/features/sidebar/entries";
import {
    GameStateMachine,
    LoadingRequirement,
    LoadingTracker,
    type StateTransition,
} from "@client/engine/game/state/index";
import { initPlayerSyncHuffman } from "@client/engine/game/sync/HuffmanProvider";
import { NpcUpdateDecoder } from "@client/engine/game/sync/NpcUpdateDecoder";
import { PlayerSyncManager } from "@client/engine/game/sync/PlayerSyncManager";
...
import { VarcPersistence } from "@client/engine/game/vars/VarcPersistence";
import { NotificationDisplay } from "@client/engine/game/widgets/NotificationDisplay";
import { PlayerDesignController } from "@client/engine/game/widgets/PlayerDesignController";
import { SpellSelectionController } from "@client/engine/game/widgets/SpellSelectionController";
import {
    type SelectedSpellInfo,
    type SpellSelectionState,
    WidgetActionRouter,
} from "@client/engine/game/widgets/WidgetActionRouter";
import { WidgetInputController } from "@client/engine/game/widgets/WidgetInputController";
import { WidgetInteractionController } from "@client/engine/game/widgets/WidgetInteractionController";
import { WidgetTransmitProcessor } from "@client/engine/game/widgets/WidgetTransmitProcessor";
import { ItemSpawnerUi } from "@client/features/item-spawner/index";
...
import { WorldMapController, type WorldMapRenderedIcon } from "@client/engine/game/world-map/WorldMapController";
import { WorldViewManager } from "@client/engine/game/worldview/WorldViewManager";
...
export class OsrsClient {
    ...
    worldEntityTypeLoader?: import("@august/osrs-engine/config/worldentitytype/WorldEntityTypeLoader").WorldEntityTypeLoader;
    ...
    modelLoader!: import("@august/osrs-engine/model/ModelLoader").ModelLoader;
    ...
    private initCacheDependent(): void {
        ...
        this.cs2Vm = new Cs2Vm({
            ...
            sendIfClose: () => {
                const pkt = createPacket(OsrsClientPacketId.IF_CLOSE);
                queuePacket(pkt);
            },
            getWeight: () => {
            ...
            sendResumePauseButton: (widgetUid: number, childIndex: number) => {
                ...
                const pkt = createPacket(OsrsClientPacketId.RESUME_PAUSEBUTTON);
    ...
    async routePlayerTo(tileX: number, tileY: number, running: boolean): Promise<void> {
        ...
        if (isServerConnected()) {
            const node = createPacket(OsrsClientPacketId.MOVE_GAMECLICK);
    ...
    private spawnNpcBinary(
        spawn: import("@client/engine/game/sync/NpcUpdateDecoder").NpcSpawn,
        loopCycle: number,
    ): void {
    ...
    private applyNpcMovementBinary(
        move: import("@client/engine/game/sync/NpcUpdateDecoder").NpcMovement,
        _loopCycle: number,
    ): void {
    ...
    private applyNpcBlocksBinary(
        npcId: number,
        block: import("@client/engine/game/sync/NpcUpdateDecoder").NpcUpdateBlock,
        loopCycle: number,
    ): void {
    ...
    private applyWorldEntityMask(
        entityIndex: number,
        entity: import("@client/engine/game/worldview/WorldEntity").WorldEntity,
        mask: import("@client/core/network/ServerConnection").WorldEntityMaskPayload,
    ): void {
