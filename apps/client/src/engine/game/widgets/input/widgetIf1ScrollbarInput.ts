...
/** Handles legacy IF1 scrollbars; UIKit panels have dedicated controllers. */
export function processWidgetIf1ScrollbarInput(
    _deps: WidgetInputControllerDeps,
    ...
    if (widgetInteraction.isDraggingWidget) return;
    ...
    if (!isLeftHeld && if1WheelDelta === 0) return;
        ...
        if (isIf1Scrollable) {
            ...
            if (isLeftHeld) {
                ...
                if (
                    ...
                    mx < scrollbarX + SCROLLBAR_WIDTH + state.if1AlternativeScrollbarWidth &&
                    my >= absY + ARROW_HEIGHT &&
                    my < absY + widgetHeight - ARROW_HEIGHT
                ) {
                    ...
                    widget.scrollY = clampScrollY(
                        trackHeight > 0
                            ? Math.floor(
                                  (clickPosY * (scrollHeight - widgetHeight)) / trackHeight,
                              )
                            : 0,
                    );
                    widgetManager.invalidateScroll(widget);
            ...
            if (
                ...
                widget.scrollY = clampScrollY((widget.scrollY ?? 0) + if1WheelDelta * 45);
                widgetManager.invalidateScroll(widget);
                handledWheel = true;
            }
        }
