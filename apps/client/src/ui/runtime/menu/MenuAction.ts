...
export function menuAction(
    ...
    if (opcode === MenuOpcode.PlayerFourthOption) {
        ...
        if (player != null) {
            setVisualFeedback();
            const pkt = createPacket(OsrsClientPacket.OPPLAYER4);
    ...
    if (opcode === MenuOpcode.PlayerThirdOption) {
        ...
        if (player != null) {
            setVisualFeedback();
            const pkt = createPacket(OsrsClientPacket.OPPLAYER3);
    ...
    if (opcode === MenuOpcode.PlayerSixthOption) {
        ...
        if (player != null) {
            setVisualFeedback();
            const pkt = createPacket(OsrsClientPacket.OPPLAYER6);
            pkt.packetBuffer.writeShort(identifier);
            pkt.packetBuffer.writeByteNeg(ctrlHeld ? 1 : 0);
            queuePacket(pkt);
        }
    ...
    if (opcode === MenuOpcode.PlayerFifthOption) {
        ...
        if (player != null) {
            setVisualFeedback();
            const pkt = createPacket(OsrsClientPacket.OPPLAYER5);
    ...
    if (opcode === MenuOpcode.PlayerEighthOption) {
        ...
        if (player != null) {
            setVisualFeedback();
            const pkt = createPacket(OsrsClientPacket.OPPLAYER8);
    ...
    if (opcode === MenuOpcode.PlayerFirstOption) {
        ...
        if (player != null) {
            setVisualFeedback();
            const pkt = createPacket(OsrsClientPacket.OPPLAYER1);
            pkt.packetBuffer.writeByteSub(ctrlHeld ? 1 : 0);
            pkt.packetBuffer.writeShort(identifier);
            queuePacket(pkt);
        }
    ...
    if (opcode === MenuOpcode.PlayerSecondOption) {
        ...
        if (player != null) {
            setVisualFeedback();
            const pkt = createPacket(OsrsClientPacket.OPPLAYER2);
            pkt.packetBuffer.writeByte(ctrlHeld ? 1 : 0);
            pkt.packetBuffer.writeShort(identifier);
            queuePacket(pkt);
        }
    ...
    if (opcode === MenuOpcode.PlayerSeventhOption) {
        ...
        if (player != null) {
            setVisualFeedback();
            const pkt = createPacket(OsrsClientPacket.OPPLAYER7);
            pkt.packetBuffer.writeShortAdd(identifier);
            pkt.packetBuffer.writeByteSub(ctrlHeld ? 1 : 0);
            queuePacket(pkt);
        }
    ...
    if (
        ...
        if (npc != null) {
            ...
            if (opcode === MenuOpcode.NpcFirstOption) {
                const pkt = createPacket(OsrsClientPacket.OPNPC1_ALT);
                pkt.packetBuffer.writeByte(ctrlHeld ? 1 : 0);
                pkt.packetBuffer.writeShortAddLE(identifier);
                queuePacket(pkt);
            } else if (opcode === MenuOpcode.NpcSecondOption) {
                const pkt = createPacket(OsrsClientPacket.OPNPC2);
                pkt.packetBuffer.writeShortAddLE(identifier);
                pkt.packetBuffer.writeByte(ctrlHeld ? 1 : 0);
                queuePacket(pkt);
            } else if (opcode === MenuOpcode.NpcThirdOption) {
                const pkt = createPacket(OsrsClientPacket.OPNPC3);
                pkt.packetBuffer.writeShortAdd(identifier);
                pkt.packetBuffer.writeByteNeg(ctrlHeld ? 1 : 0);
                queuePacket(pkt);
            } else if (opcode === MenuOpcode.NpcFourthOption) {
                const pkt = createPacket(OsrsClientPacket.OPNPC4);
                pkt.packetBuffer.writeByteNeg(ctrlHeld ? 1 : 0);
                pkt.packetBuffer.writeShortLE(identifier);
                queuePacket(pkt);
            } else {
                const pkt = createPacket(OsrsClientPacket.OPNPC1);
    ...
    if (opcode === MenuOpcode.GameObjectFirstOption) {
        ...
        const pkt = createPacket(OsrsClientPacket.OPLOC1);
    ...
    if (opcode === MenuOpcode.GameObjectSecondOption) {
        ...
        const worldY = (ClientState.baseY | 0) + (arg1 | 0);
        const worldX = (ClientState.baseX | 0) + (arg0 | 0);
        const pkt = createPacket(OsrsClientPacket.OPLOC2);
    ...
    if (opcode === MenuOpcode.GameObjectThirdOption) {
        ...
        const worldY = (ClientState.baseY | 0) + (arg1 | 0);
        const worldX = (ClientState.baseX | 0) + (arg0 | 0);
        const pkt = createPacket(OsrsClientPacket.OPLOC3);
        pkt.packetBuffer.writeShortLE(worldY);
        pkt.packetBuffer.writeShortLE(identifier);
        pkt.packetBuffer.writeShortAddLE(worldX);
        pkt.packetBuffer.writeByteSub(ctrlHeld ? 1 : 0);
    ...
    if (opcode === MenuOpcode.GameObjectFourthOption) {
        ...
        const worldY = (ClientState.baseY | 0) + (arg1 | 0);
        const worldX = (ClientState.baseX | 0) + (arg0 | 0);
        const pkt = createPacket(OsrsClientPacket.OPLOC4);
        pkt.packetBuffer.writeShortAdd(worldX);
        pkt.packetBuffer.writeShortLE(identifier);
    ...
    if (opcode === MenuOpcode.GameObjectFifthOption) {
        ...
        const worldY = (ClientState.baseY | 0) + (arg1 | 0);
        const worldX = (ClientState.baseX | 0) + (arg0 | 0);
        const pkt = createPacket(OsrsClientPacket.OPLOC5);
    ...
    if (opcode === MenuOpcode.GroundItemFirstOption) {
        ...
        const worldY = (ClientState.baseY | 0) + (arg1 | 0);
        const worldX = (ClientState.baseX | 0) + (arg0 | 0);
        const pkt = createPacket(OsrsClientPacket.OPOBJ1);
        pkt.packetBuffer.writeByteSub(ctrlHeld ? 1 : 0);
        pkt.packetBuffer.writeShortLE(worldY);
        pkt.packetBuffer.writeShortAdd(identifier);
        pkt.packetBuffer.writeShortAdd(worldX);
    ...
    if (opcode === MenuOpcode.GroundItemSecondOption) {
        ...
        const worldY = (ClientState.baseY | 0) + (arg1 | 0);
        const worldX = (ClientState.baseX | 0) + (arg0 | 0);
        const pkt = createPacket(OsrsClientPacket.OPOBJ2);
        pkt.packetBuffer.writeShortAdd(worldX);
        pkt.packetBuffer.writeShortLE(worldY);
        pkt.packetBuffer.writeShort(identifier);
        pkt.packetBuffer.writeByte(ctrlHeld ? 1 : 0);
    ...
    if (opcode === MenuOpcode.GroundItemThirdOption) {
        ...
        const worldY = (ClientState.baseY | 0) + (arg1 | 0);
        const worldX = (ClientState.baseX | 0) + (arg0 | 0);
        const pkt = createPacket(OsrsClientPacket.OPOBJ3);
        pkt.packetBuffer.writeShortLE(identifier);
        pkt.packetBuffer.writeShortAdd(worldX);
        pkt.packetBuffer.writeShortAddLE(worldY);
        pkt.packetBuffer.writeByteSub(ctrlHeld ? 1 : 0);
    ...
    if (opcode === MenuOpcode.GroundItemFourthOption) {
        ...
        const worldY = (ClientState.baseY | 0) + (arg1 | 0);
        const worldX = (ClientState.baseX | 0) + (arg0 | 0);
        const pkt = createPacket(OsrsClientPacket.OPOBJ4);
        pkt.packetBuffer.writeShortAddLE(identifier);
        pkt.packetBuffer.writeShortAddLE(worldY);
        pkt.packetBuffer.writeByteNeg(ctrlHeld ? 1 : 0);
        pkt.packetBuffer.writeShortAdd(worldX);
    ...
    if (opcode === MenuOpcode.GroundItemFifthOption) {
        ...
        const worldY = (ClientState.baseY | 0) + (arg1 | 0);
        const worldX = (ClientState.baseX | 0) + (arg0 | 0);
        const pkt = createPacket(OsrsClientPacket.OPOBJ5);
        pkt.packetBuffer.writeShortAdd(identifier);
    ...
    if (opcode === MenuOpcode.ItemUseOnGameObject) {
        ...
        const worldY = (ClientState.baseY | 0) + (arg1 | 0);
        const worldX = (ClientState.baseX | 0) + (arg0 | 0);
        const pkt = createPacket(OsrsClientPacket.OPLOCU);
    ...
    if (opcode === MenuOpcode.WidgetTargetOnGameObject) {
        ...
        const worldY = (ClientState.baseY | 0) + (arg1 | 0);
        const worldX = (ClientState.baseX | 0) + (arg0 | 0);
        const pkt = createPacket(OsrsClientPacket.OPLOCT);
        pkt.packetBuffer.writeShortAddLE(worldY);
        pkt.packetBuffer.writeShortAdd(identifier);
    ...
    if (opcode === MenuOpcode.ItemUseOnNpc) {
        ...
        if (npc != null) {
            setVisualFeedback();
            const pkt = createPacket(OsrsClientPacket.OPNPCU);
            pkt.packetBuffer.writeShortAddLE(ClientState.selectedItemSlot);
            pkt.packetBuffer.writeByte(ctrlHeld ? 1 : 0);
    ...
    if (opcode === MenuOpcode.WidgetTargetOnNpc) {
        ...
        if (npc != null) {
            normalizeSelectedSpellState();
            setVisualFeedback();
            const pkt = createPacket(OsrsClientPacket.OPNPCT);
            pkt.packetBuffer.writeShort(identifier);
            pkt.packetBuffer.writeIntLE(ClientState.selectedSpellWidget);
    ...
    if (opcode === MenuOpcode.ItemUseOnPlayer) {
        ...
        if (player != null) {
            setVisualFeedback();
            const pkt = createPacket(OsrsClientPacket.OPPLAYERU);
            pkt.packetBuffer.writeShortAddLE(identifier);
            pkt.packetBuffer.writeShortAddLE(ClientState.selectedItemId);
    ...
    if (opcode === MenuOpcode.WidgetTargetOnPlayer) {
        ...
        if (player != null) {
            normalizeSelectedSpellState();
            setVisualFeedback();
            const pkt = createPacket(OsrsClientPacket.OPPLAYERT);
            pkt.packetBuffer.writeByteNeg(ctrlHeld ? 1 : 0);
            pkt.packetBuffer.writeShortLE(ClientState.selectedSpellItemId);
    ...
    if (opcode === MenuOpcode.ItemUseOnGroundItem) {
        ...
        const worldY = (ClientState.baseY | 0) + (arg1 | 0);
        const worldX = (ClientState.baseX | 0) + (arg0 | 0);
        const pkt = createPacket(OsrsClientPacket.OPOBJU);
        pkt.packetBuffer.writeShortAddLE(identifier);
        pkt.packetBuffer.writeShortAdd(worldY);
        pkt.packetBuffer.writeIntME(ClientState.selectedItemWidget);
        pkt.packetBuffer.writeShortAdd(worldX);
    ...
    if (opcode === MenuOpcode.WidgetTargetOnGroundItem) {
        ...
        const worldY = (ClientState.baseY | 0) + (arg1 | 0);
        const worldX = (ClientState.baseX | 0) + (arg0 | 0);
        const pkt = createPacket(OsrsClientPacket.OPOBJT);
        pkt.packetBuffer.writeIntLE(ClientState.selectedSpellWidget);
        pkt.packetBuffer.writeShortAdd(ClientState.selectedSpellChildIndex);
        pkt.packetBuffer.writeShortAdd(identifier);
        pkt.packetBuffer.writeShortAddLE(worldX);
    ...
    if (opcode === MenuOpcode.WalkHere) {
        ...
        const pkt = createPacket(OsrsClientPacket.MOVE_GAMECLICK);
        pkt.packetBuffer.writeShortAddLE(worldY);
    ...
    if (opcode === MenuOpcode.WidgetType1) {
        // arg1 = widget ID (parent << 16 | child)
        const pkt = createPacket(OsrsClientPacket.IF_BUTTON);
    ...
    if (opcode === MenuOpcode.WidgetClose) {
        ...
        const pkt = createPacket(OsrsClientPacket.IF_CLOSE);
        queuePacket(pkt);
    ...
    if (opcode === MenuOpcode.WidgetContinue) {
        ...
        const pkt = createPacket(OsrsClientPacket.RESUME_PAUSEBUTTON);
    ...
    if (opcode === MenuOpcode.CC_OP || opcode === MenuOpcode.CC_OP_LowPriority) {
        ...
        if (opIndex >= 1 && opIndex <= 10) {
            ...
            switch (opIndex) {
                case 1:
                    packetType = OsrsClientPacket.IF_BUTTON1;
                    break;
                case 2:
                    packetType = OsrsClientPacket.IF_BUTTON2;
                    break;
                case 3:
                    packetType = OsrsClientPacket.IF_BUTTON3;
                    break;
                case 4:
                    packetType = OsrsClientPacket.IF_BUTTON4;
                    break;
                case 5:
                    packetType = OsrsClientPacket.IF_BUTTON5;
                    break;
                case 6:
                    packetType = OsrsClientPacket.IF_BUTTON6;
                    break;
                case 7:
                    packetType = OsrsClientPacket.IF_BUTTON7;
                    break;
                case 8:
                    packetType = OsrsClientPacket.IF_BUTTON8;
                    break;
                case 9:
                    packetType = OsrsClientPacket.IF_BUTTON9;
                    break;
                case 10:
                    packetType = OsrsClientPacket.IF_BUTTON10;
                    break;
                default:
                    packetType = OsrsClientPacket.IF_BUTTON1;
                    break;
    ...
    if (opcode === MenuOpcode.WidgetTargetOnWidget) {
        ...
        setVisualFeedback(false);
        const pkt = createPacket(OsrsClientPacket.IF_BUTTONT);
    ...
    if (opcode === MenuOpcode.ExamineObject) {
        setVisualFeedback(false);
        const pkt = createPacket(OsrsClientPacket.EXAMINE_LOC);
        pkt.packetBuffer.writeShortAddLE(identifier);
        queuePacket(pkt);
    }
    ...
    if (opcode === MenuOpcode.ExamineGroundItem) {
        ...
        const worldY = (ClientState.baseY | 0) + (arg1 | 0);
        const worldX = (ClientState.baseX | 0) + (arg0 | 0);
        const pkt = createPacket(OsrsClientPacket.EXAMINE_OBJ);
        pkt.packetBuffer.writeShort(identifier);
        pkt.packetBuffer.writeShortLE(worldY);
        pkt.packetBuffer.writeShortLE(worldX);
        queuePacket(pkt);
