...
export class WidgetInputBridge {
...
/**
 * Gets or creates the input bridge owned by a GL renderer canvas.
 * Stores the bridge on the canvas for reuse across frames.
 */
export function ensureWidgetInputBridge(glr: GLRenderer): WidgetInputBridge {
    const canvas = glr.canvas as any;
    if (!canvas.__inputBridge) {
        canvas.__inputBridge = new WidgetInputBridge(glr.canvas);
    }
