...
const resolvedActiveWorld = worlds.find((world) => world.id === selectedWorldId);

if (!resolvedActiveWorld) {
    throw new Error(`Unknown world ID ${selectedWorldId}. Configure it in server/config.json.`);
}

export const activeWorld: WorldConfig = resolvedActiveWorld;
...
export const config: ServerConfig = {
    ...
    serverName: process.env.SERVER_NAME?.trim() || activeWorld.name,
    maxPlayers,
    worlds,
};
