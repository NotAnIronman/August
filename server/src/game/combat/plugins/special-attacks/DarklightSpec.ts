...
/**
 * Weaken rolls against Stab defence and, on a successful hit, drains Attack,
 * Strength, and Defence by 5% of each base level plus one. Demons use a 10%
 * fraction; that multiplier requires an NPC demon-category flag, which is not
 * exposed by the current NPC combat state.
 */
export class DarklightSpec implements WeaponSpecialAttackScript {
    readonly itemId = DARKLIGHT_ITEM_ID;
...
export const DARKLIGHT_SPEC = Object.freeze(new DarklightSpec());
