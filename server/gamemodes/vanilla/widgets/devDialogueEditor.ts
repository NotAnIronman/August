...
import { DEV_UIKIT_DIALOGUE_PANEL_GROUP_ID } from "../../../../client/common/ui/widgets/custom/journalPanel.cs2";
import { ComponentIds, type UiTextRow } from "../../../../client/common/uikit/contracts";
import type {
    DialogueAction,
    DialogueCondition,
    DialogueComparator,
    ...
    DialogueStep,
    DialogueTreeJson,
} from "../../../src/game/dialogue/DialogueTree";
import type { PlayerState } from "../../../src/game/player";
import type { CommandEvent, IScriptRegistry, ScriptServices } from "../../../src/game/scripts/types";
import { registerUiPanelActions, type UiPanelAction } from "../uikit/actions";
import {
...
export function registerDevDialogueEditor(registry: IScriptRegistry, services: ScriptServices): void {
    registry.registerCommand("editdialogue", (event) => {
    ...
    registry.registerCommand("dsel", (event) => {
     ...
     *  in one step. */
    registry.registerCommand("dedit", (event) => {
        const state = requireState(event, services);
        if (typeof state === "string") return state;
        const path = event.args[0];
    ...
    registry.registerCommand("dinput", (event) => {
        const state = requireState(event, services);
        if (typeof state === "string") return state;
    ...
    registry.registerCommand("dline", (event) => {
        const state = requireState(event, services);
        if (typeof state === "string") return state;
    ...
    registry.registerCommand("doption", (event) => {
        const state = requireState(event, services);
        if (typeof state === "string") return state;
    ...
    registry.registerCommand("dnest", (event) => {
        const state = requireState(event, services);
        if (typeof state === "string") return state;
    ...
    registry.registerCommand("dcond", (event) => {
        const state = requireState(event, services);
        if (typeof state === "string") return state;
    ...
    registry.registerCommand("daction", (event) => {
        const state = requireState(event, services);
        if (typeof state === "string") return state;
    ...
    registry.registerCommand("dtext", (event) => {
        const state = requireState(event, services);
        if (typeof state === "string") return state;
    ...
    registry.registerCommand("dspeaker", (event) => {
        const state = requireState(event, services);
        if (typeof state === "string") return state;
    ...
    registry.registerCommand("dmove", (event) => {
        const state = requireState(event, services);
        if (typeof state === "string") return state;
    ...
    registry.registerCommand("ddelete", (event) => {
        const state = requireState(event, services);
        if (typeof state === "string") return state;
    ...
    registry.registerCommand("ddeletebranch", (event) => {
        const state = requireState(event, services);
        if (typeof state === "string") return state;
