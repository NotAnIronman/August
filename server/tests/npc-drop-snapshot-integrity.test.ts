...
import type { NpcDropTable } from "../src/game/drops/types";
...
const runtimePath = path.join(root, "server", "data", "npc-drops-wiki.json");
const bootstrapPath = path.join(root, "references", "npc-drops-wiki.json");

for (const sourcePath of [runtimePath, bootstrapPath]) {
    const text = fs.readFileSync(sourcePath, "utf8");
    assert.equal(
        /"itemId"\s*:\s*617\b/.test(text),
        false,
        `${path.relative(root, sourcePath)} must not contain non-canonical coin drops`,
    );
}

const snapshot = JSON.parse(fs.readFileSync(runtimePath, "utf8")) as {
...
assert(liveKrilTable, "K'ril's canonical runtime table must resolve");
...
function rollKrilOutcome(itemId: number) {
    ...
    Math.random = () => randomForWeightedOutcome(liveKrilTable, itemId);
    try {
