 ...
 *
 * Usage:
 *   yarn --cwd server sync-missing-cache-items [cacheName] [--dry-run]
 */
import fs from "fs";
...
function readExistingItemIds(text: string): Set<number> {
    ...
    if (!Array.isArray(parsed)) throw new Error("server/data/items.json must contain an array");

    const ids = new Set<number>();
    for (const entry of parsed) {
        const id = (entry as Partial<ExistingItemRecord>)?.id;
        if (typeof id !== "number" || !Number.isInteger(id) || id < 0) {
            throw new Error("server/data/items.json contains an item without a non-negative integer id");
        }
        if (ids.has(id)) throw new Error(`server/data/items.json contains duplicate item id ${id}`);
        ids.add(id);
    }
    return ids;
}
...
function appendRecords(text: string, records: readonly ServerItemRecord[]): string {
    ...
    if (closeIndex < 0 || text.slice(closeIndex + 1).trim().length !== 0) {
        throw new Error("server/data/items.json is not a simple top-level JSON array");
    }
...
function main(): void {
    ...
    console.log(
        "[sync-missing-cache-items] Next run: yarn --cwd server import-equipment-reference",
    );
}
