...
const NPCS_PATH = path.join(ROOT, "server", "data", "npcs.json");
const NPC_AGGRESSION_PATH = path.join(ROOT, "server", "data", "npc-aggression.json");
...
function loadExportedNpcTargets(
    ...
    if (!Array.isArray(parsed)) throw new Error("server/data/npcs.json must contain an array");
    const targets = new Map<number, NpcTarget>();
...
function loadCacheNpcTargets(
    ...
    try {
        ...
        console.warn(
            `[sync-npc-drops] Active cache unavailable; using server/data/npcs.json: ${
