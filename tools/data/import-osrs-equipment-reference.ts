/**
 * Refreshes equipment bonuses in `data/generated/server/items.json` from the published
 ...
 *
 * Usage:
 *   pnpm --filter @august/server import-equipment-reference
 *   pnpm --filter @august/server import-equipment-reference -- --dry-run
 *   pnpm --filter @august/server import-equipment-reference -- --source path/to/equipment.json
 */
import fs from "fs";
...
const LOCAL_REQUIREMENTS_PATH = referencePath("osrs-equipment-requirements.json");
...
async function main(): Promise<void> {
    ...
    if (!Array.isArray(localItems)) throw new Error("Generated items snapshot must be an array");
