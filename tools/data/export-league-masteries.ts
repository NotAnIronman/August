...
function formatLeagueMasteriesTs(
    ...
    return (
        `import type { LeagueRelicRow, LeagueMasteryNodeRow, LeagueMasteryChallengeRow } from "@august/protocol/gamemode/GamemodeDataTypes";\n\n` +
    ...
    if (leagueTypeEnum) {
        ...
        for (const structId of leagueTypeEnum.intValues || []) {
            ...
            for (const [paramId, value] of struct.params.entries()) {
                ...
                const enumId = value;
                if (
                    typeof enumId === "number" &&
                    Number.isFinite(enumId) &&
                    enumId > 100 &&
                    enumId < 50000
                ) {
...
function main(): void {
    ...
    if (relics.length > 0 || masteryNodes.length > 0 || masteryChallenges.length > 0) {
        const outPath = serverContentPath(
            "gamemodes",
            "leagues-v",
            "data",
