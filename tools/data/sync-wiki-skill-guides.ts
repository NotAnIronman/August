 ...
 *
 * Usage:
 *   pnpm --filter @august/server sync-wiki-skill-guides
 *   pnpm --filter @august/server sync-wiki-skill-guides -- --dry-run
 *   pnpm --filter @august/server sync-wiki-skill-guides -- --skill fishing
 *   pnpm --filter @august/server sync-wiki-skill-guides -- --source-dir data/references/skill-guide
 */
import fs from "fs";
import path from "path";
import { serverContentPath, serverGeneratedDataPath } from "@tools/lib/repository-paths";

const ITEMS_PATH = serverGeneratedDataPath("items.json");
const DATA_DIR = serverContentPath("gamemodes", "vanilla", "skillGuide", "data");
...
type GuideEntry = {
    ...
    name: string;
    description?: string;
    /** Intentionally seeded for quick hand-authoring after an import. */
    requires: string;
};
...
function generateFile(skillId: string, tabs: readonly GuideTab[]): string {
    return `// Generated by scripts/sync-wiki-skill-guides.ts from the OSRS Wiki's public\n` +
        `// ${skillId}/Level_up_table page. The blank requires fields are intended for hand editing.\n` +
