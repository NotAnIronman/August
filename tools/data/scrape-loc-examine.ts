/**
 * Fills data/generated/server/locs.json examine text via two passes:
 ...
 *
 * Run with: pnpm --filter @august/server scrape-loc-examine [--limit=N] [--resume]
...
import { cacheGeneratedDataPath, serverGeneratedDataPath } from "@tools/lib/repository-paths";
...
function parseArgs(): { limit: number | undefined; resume: boolean; locsSrc: string } {
    ...
    return {
        ...
        locsSrc: path.resolve(
            locsFlag ? locsFlag.slice("--locs=".length) : cacheGeneratedDataPath("locs.json"),
        ),
    };
}
...
async function main() {
    ...
    if (!fs.existsSync(locsSrc)) {
        console.error(`[scrape-loc-examine] ${locsSrc} not found — run the client loc cache exporter first.`);
        process.exit(1);
    }
