/**
 * Fills blank `examine` fields in the canonical generated loc/NPC snapshots.
 ...
 *
 * Run with: pnpm --filter @august/server fill-blank-examine [--locs] [--npcs]
 * (defaults to both if neither flag is given)
 */
import fs from "fs";
import { serverGeneratedDataPath } from "@tools/lib/repository-paths";
...
function main() {
    ...
    if (doLocs) processFile(serverGeneratedDataPath("locs.json"), "locs");
    if (doNpcs) processFile(serverGeneratedDataPath("npcs.json"), "npcs");
