 ...
 *
 * Run with:
 *   pnpm --filter @august/server import-examine-wiki
 *
 * Reads data/generated/cache/{npcs,locs}.json by default
 * (where the client cache exporters write them) —
 * override with --npcs=<path> / --locs=<path> if yours are elsewhere.
 */
import fs from "fs";
import path from "path";

import { ExamineOverrideStore } from "@server/game/examine/ExamineOverrideStore";
import { getGamemodeDataDir } from "@server/game/gamemodes/GamemodeRegistry";
import { getSqliteDatabase } from "@server/game/state/SqliteDatabase";
import { cacheGeneratedDataPath } from "@tools/lib/repository-paths";
...
function parseArgs(): { npcsPath: string; locsPath: string; targetGamemode: string } {
    ...
    return {
        npcsPath: path.resolve(get("npcs", cacheGeneratedDataPath("npcs.json"))),
        locsPath: path.resolve(get("locs", cacheGeneratedDataPath("locs.json"))),
