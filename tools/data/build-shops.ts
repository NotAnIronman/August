/**
 * Builds data/generated/server/shops.json from:
 * - DayV osrsreboxed shops-items-by-shop.json (stock + pricing)
 * - DayV shops-wiki-page-text.json (owners + map + general-store flag)
 * - data/generated/server/npc-spawns.json (npcId wiring via name + proximity)
 *
 * Inputs are downloaded into data/references/shops/ on first run.
 */
import fs from "fs";
...
const SOURCES = referencePath("shops");
const SPAWNS_PATH = serverGeneratedDataPath("npc-spawns.json");
const OUT = serverGeneratedDataPath("shops.json");
