 ...
 *
 * Run with: pnpm --filter @august/server merge-wiki-examine
 *
 * Source data:
 *  - Local cache exports at data/generated/cache/{npcs,locs}.json.
...
function parseArgs(): { npcsSrc: string; locsSrc: string } {
    ...
    return {
        npcsSrc: path.resolve(get("npcs", cacheGeneratedDataPath("npcs.json"))),
        locsSrc: path.resolve(get("locs", cacheGeneratedDataPath("locs.json"))),
    };
}

async function buildNpcs(srcPath: string): Promise<OutputDefinition[]> {
    if (!fs.existsSync(srcPath)) {
        console.warn(`[merge-wiki-examine] ${srcPath} not found — run the client NPC cache exporter first. Skipping NPCs.`);
        return [];
    }
...
async function buildLocs(srcPath: string): Promise<OutputDefinition[]> {
    if (!fs.existsSync(srcPath)) {
        console.warn(`[merge-wiki-examine] ${srcPath} not found — run the client loc cache exporter first. Skipping locs.`);
        return [];
    }
    const local = JSON.parse(fs.readFileSync(srcPath, "utf8")) as LocalDefinition[];
...
async function main() {
    ...
    if (npcs.length > 0) {
        const outPath = serverGeneratedDataPath("npcs.json");
    ...
    if (locs.length > 0) {
        const outPath = serverGeneratedDataPath("locs.json");
