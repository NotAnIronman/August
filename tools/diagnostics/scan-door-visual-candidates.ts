...
function buildLocSignature(
    ...
    const modelsRaw = Array.isArray(loc.models)
        ? loc.models
              ...
              .filter((value: unknown): value is number => finiteNumber(value, NaN) === value)
        : [];
    ...
    const types = sortedUnique(
        Array.isArray(loc.types)
            ? loc.types.filter(
                  (value: unknown): value is number => finiteNumber(value, NaN) === value,
              )
            : [],
    );
    const nameValue = loc.name as string | undefined;
    const isInteractive = finiteNumber(loc.isInteractive, 0);
    const clipType = finiteNumber(loc.clipType, 0);
    const modelSizeX = finiteNumber(loc.modelSizeX, 128);
    const modelSizeY = finiteNumber(loc.modelSizeY, 128);
    const modelSizeHeight = finiteNumber(loc.modelSizeHeight, 128);
    const offsetX = finiteNumber(loc.offsetX, 0);
    const offsetY = finiteNumber(loc.offsetY, 0);
    const offsetHeight = finiteNumber(loc.offsetHeight, 0);
...
function main(): void {
    ...
    if (opts.closedIds.length === 0) {
        console.log(
            [
                "Usage:",
                "  pnpm exec tsx tools/diagnostics/scan-door-visual-candidates.ts --closed=52,53",
                "Options:",
                "  --cache=<name>       Cache folder under apps/server/var/cache/osrs",
    ...
    const env = initCacheEnv(serverVarPath("cache", "osrs"), opts.cacheName);
