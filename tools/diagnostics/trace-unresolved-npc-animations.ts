/**
 * Trace every unresolved NPC in the manual encounter notes to its first archived OSRS cache
...
import { generatedReportPath, serverGeneratedDataPath } from "@tools/lib/repository-paths";
...
const OUTPUT_PATH = generatedReportPath(
    "npc-animations",
    "unresolved-npc-historical-animation-batches.json",
);
const MARKDOWN_OUTPUT_PATH = generatedReportPath(
    "npc-animations",
...
type Target = {
    ...
    /** Every documented form that should inherit this review batch. */
...
function loadLocalNpcIds(): Map<string, number[]> {
    const candidates = [serverGeneratedDataPath("npcs.json")];
